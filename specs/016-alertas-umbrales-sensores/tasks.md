...
| **Última revisión** | 2026-09-13 (T1-T12 cerrado por bookkeeping — decisión de T7/T8 ya existía en ADR-140 desde 2026-09-02; Art. 5 de la Constitución cerrado con métricas reales nuevas, `AlarmEngineStats`; queda solo el demo en vivo de gate R5, sin ejecutar por falta de sesión autenticada en esta pasada) |
...
- [x] T1-T12 completadas. *(2026-09-13: cerrado — 11/12 son código real; T7/T8 (canal SSE dedicado dado) NO se construyen por una decisión consciente ya tomada y documentada en ADR-140 desde 2026-09-02 ("agregar SSE en paralelo sería un segundo transporte para el mismo evento, sin pedido de negocio que lo justifique" — la plataforma ya empuja alarmas en tiempo real por WebSocket, `alarmStream.ts`/`alarm_notifier.cpp`). Este checkbox quedaba sin cerrar por un descuido de bookkeeping, no porque la decisión estuviera pendiente — se cierra hoy referenciando la decisión ya existente, sin tomar ninguna decisión nueva.)*
...
- [x] Sin violar Constitución (Art. 1, 2, 5, 9). *(2026-09-13, verificado explícitamente contra el código real, no solo por inspección superficial: **Art. 1** (multitenancy) — ya probado en vivo con 2 tenants demo reales (T18, 2026-09-02): regla de un tenant no aparece en el listado del otro, DELETE cross-tenant → 404. **Art. 2** (tiempo real sin pérdida) — verificado leyendo `telemetry_ingest.cpp`: el camino en tiempo real (`handleRealtimeTelemetryBatch`) se registra vía `setOnBatchCommitted`, invocado ÚNICAMENTE después del COMMIT real de Postgres del lote ("batch ya está COMMITeado (durable) en este punto", comentario del propio código) — nunca evalúa contra datos que pudieran hacer ROLLBACK; el polling de 10s (`evaluateRulesOnce`) es una red de seguridad redundante para el mismo objetivo. **Art. 5** (observabilidad obligatoria) — GAP REAL encontrado y corregido hoy: el motor de alarmas no exponía ninguna métrica en `/api/metrics` pese a estar corriendo en producción desde ADR-034 (2026-06-24). Se agregó `mining_iot::AlarmEngineStats` (`device_alarm_routes.hpp/cpp`) con contadores reales (`evaluations`, `triggered`, `debounced`, `resolved`, `rules_cached`) e integrado a `handleMetrics` (`main.cpp`) como `beemetry_alarm_engine_*`. Verificado desplegado: build real (CTest 1/1 passed), `docker compose build/up web`, `curl http://127.0.0.1:8082/api/metrics` confirma las 5 métricas nuevas expuestas. **Art. 9** (recursos acotados) — anti-tormenta cubierto por T5 (debounce), sin cambios.)*
- [ ] Gate R5: demo alerta en tiempo real en dashboard. *(2026-09-13, segunda pasada — avance real, sigue sin cerrar por un motivo distinto y más preciso que la vez anterior. Esta vez SÍ había un navegador autenticado disponible (sesión real de "larmas111"/Alpayana, persistida entre turnos) y se intentó ejecutar el demo de punta a punta:*
  1. *Se investigó por qué el camino de polling podría fallar aunque hubiera una regla — **se encontró y corrigió un bug real de rendimiento**: la consulta de "último valor" para reglas con `sensor_id` real no tenía cota de tiempo y, con los ~10.900 chunks que `telemetry_fact` ya acumuló (`chunk_time_interval=1h`), no completaba ni con un `statement_timeout` de 10s — la "red de seguridad" de polling del Art. 2 estaba, en la práctica, rota para cualquier regla con `sensor_id` real. Se encontró el MISMO bug en `GET /api/mining/telemetry/summary` (dashboard general, confirmado colgado 45s+ reales) y en `simulation_status_routes.cpp`. Los 3 se corrigieron acotando la ventana de tiempo (15 min / 24h según el caso), medido antes→después, build real + deploy real. Ver [ADR-186](../../docs/decisions/186-bug-real-timescaledb-chunk-scan-sin-cota-tiempo.md).*
  2. *Se intentó crear una regla real (`POST /api/mining/alarms/rules`, `mining_sensor_id=3` "POLVO-STACK-A", umbral ya cruzado por su `current_value` real) usando la sesión autenticada disponible — **bloqueado por RBAC**: el usuario de esa sesión tiene rol `viewer` (confirmado: `403 forbidden, need: alarmas.manage`). Esto es el sistema de permisos funcionando correctamente, no un bug — un `viewer` no debe poder crear reglas de alarma.*
  *En limpio: el mecanismo de evaluación quedó más verificado y más rápido que al inicio de esta pasada (bug real corregido), pero la demo visual en el dashboard sigue sin ejecutarse — ahora el único bloqueo conocido es de permisos, no de código ni de disponibilidad de sesión. **Para cerrar esto falta**: repetir el mismo intento con una cuenta que tenga el rol/permiso `alarmas.manage` (o que el usuario lo haga desde su propia sesión con el rol correcto) — el resto de los pasos (esperar el ciclo de 10s, confirmar en el dashboard) no requieren nada adicional.)*
