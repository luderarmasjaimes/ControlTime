...
/* Extensiones Beemetry sobre el tinyexpr original: iif/max/min/round.
   Necesarias para poder migrar reglas de alerta del ThingsBoard legado
   (umbrales por rango, clamps) sin control de flujo real -- ver
   scripts/translate_thingsboard_formulas.py y ADR-197/198. iif() evalúa
   AMBAS ramas de forma eager (tinyexpr no tiene short-circuit de funciones)
   y simplemente selecciona el resultado ya calculado; eso es seguro para
   fórmulas puras sin efectos secundarios como las de este motor -- un NaN
   en la rama descartada no se propaga porque nunca se usa su valor. */
static double te_iif(double cond, double a, double b) {return cond != 0.0 ? a : b;}
static double te_max(double a, double b) {return a > b ? a : b;}
static double te_min(double a, double b) {return a < b ? a : b;}
static double te_round(double a) {return floor(a + 0.5);}
...
static const te_variable functions[] = {
    ...
    {"iif", te_iif,   TE_FUNCTION3 | TE_FLAG_PURE, 0},
    ...
    {"max", te_max,   TE_FUNCTION2 | TE_FLAG_PURE, 0},
    {"min", te_min,   TE_FUNCTION2 | TE_FLAG_PURE, 0},
    {"ncr", ncr,      TE_FUNCTION2 | TE_FLAG_PURE, 0},
    {"npr", npr,      TE_FUNCTION2 | TE_FLAG_PURE, 0},
    {"pi", pi,        TE_FUNCTION0 | TE_FLAG_PURE, 0},
    {"pow", pow,      TE_FUNCTION2 | TE_FLAG_PURE, 0},
    {"round", te_round, TE_FUNCTION1 | TE_FLAG_PURE, 0},
