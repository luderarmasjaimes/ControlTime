...
/** Resuelve tenants.tenant_id para telemetría: mineria_empresas → auth_user_tenant → demo fijo. */
#if HAS_LIBPQ
static std::string resolveTelemetryTenantIdPg(void *connV, const std::string &userId,
                                              const std::string &companyName) {
...
#else
static std::string resolveTelemetryTenantIdPg(void * /*connV*/,
                                              const std::string & /*userId*/,
                                              const std::string & /*companyName*/) {
  return std::string(kMiningTelemetryDemoTenantId);
}
#endif
...
#if HAS_LIBPQ
static json::array listCompanyUsersPg(const std::string &databaseUrl, const std::string &company) {
  json::array users;
  PGconn *conn = PQconnectdb(databaseUrl.c_str());
  if (PQstatus(conn) != CONNECTION_OK) {
    PQfinish(conn);
...
static json::array listUserMaintenanceAuditPg(const std::string &databaseUrl, const std::string &company, int page, int pageSize) {
  json::array logs;
  PGconn *conn = PQconnectdb(databaseUrl.c_str());
  if (PQstatus(conn) != CONNECTION_OK) {
    PQfinish(conn);
...
static bool executeUserMaintenancePg(const std::string &databaseUrl, const json::object &payload, json::object &outAudit, std::string &error) {
  PGconn *conn = PQconnectdb(databaseUrl.c_str());
  if (PQstatus(conn) != CONNECTION_OK) {
    error = PQerrorMessage(conn);
    PQfinish(conn);
  ...
  try {
    ...
    // Operator validation (simplified here, should check password/token usually)
...
routeRequest(const http::request<http::string_body> &req,
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/text/correct/quick") {
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/text/correct/advanced") {
    const auto session = resolveAuthSession(req, query);
    if (!session) {
      return makeJsonResponse(http::status::unauthorized,
                              json::object{{"error", "unauthorized"}});
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/text/rewrite") {
    const auto session = resolveAuthSession(req, query);
    if (!session) {
      return makeJsonResponse(http::status::unauthorized,
                              json::object{{"error", "unauthorized"}});
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/text/languagetool-check") {
    const auto session = resolveAuthSession(req, query);
    if (!session) {
      return makeJsonResponse(http::status::unauthorized,
                              json::object{{"error", "unauthorized"}});
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/platform/countries") {
    json::array items;
#if HAS_LIBPQ
    PGconn *conn = PQconnectdb(gDatabaseUrl.c_str());
    if (PQstatus(conn) == CONNECTION_OK) {
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/platform/ui-languages") {
    json::array items;
#if HAS_LIBPQ
    PGconn *conn = PQconnectdb(gDatabaseUrl.c_str());
    if (PQstatus(conn) == CONNECTION_OK) {
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/config/mining-locations") {
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/mining/kpis") {
    json::array rows;
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/mining/kpis/upsert") {
    const auto session = resolveAuthSession(req, query);
    if (!session) {
    ...
    try {
      ...
      for (const auto &v : arr) {
        ...
        const std::string category = json::value_to<std::string>(it.at("category"));
        ...
        std::string sql =
            ...
            pqEscapeLiteral(conn, code) + "," + pqEscapeLiteral(conn, category) + "," +
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/mining/kpis/sync-from-dashboard") {
    if (gAuthStorageMode != AuthStorageMode::Postgres) {
      return makeJsonResponse(http::status::bad_request, json::object{{"error", "postgres_required"}});
    }
#if HAS_LIBPQ
    ...
    for (int i = 0; i < PQntuples(src); ++i) {
      ...
      std::string upsertSql =
          ...
          pqEscapeLiteral(conn, name) + ", " + pqEscapeLiteral(conn, category) + ", " +
  ...
  if (req.method() == http::verb::post && pathOnly == "/api/mining/kpis/sync-from-external") {
    if (gAuthStorageMode != AuthStorageMode::Postgres) {
      return makeJsonResponse(http::status::ok,
    ...
    for (int i = 0; i < PQntuples(extRows); ++i) {
      ...
      const std::string category = PQgetvalue(extRows, i, 1);
      ...
      std::string upsertSql =
          ...
          "(code, category, title, description, unit, current_value, target_value, trend_direction, trend_percent, "
          "status_color, active, sort_order, updated_at) VALUES (" +
          pqEscapeLiteral(localConn, code) + ", " + pqEscapeLiteral(localConn, category) + ", " +
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/mining/kpis/points") {
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/surveillance/camera-snapshot") {
    const auto session = resolveAuthSession(req, query);
    if (!session) {
      return makeJsonResponse(http::status::unauthorized,
                              json::object{{"error", "unauthorized"}});
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/map/official-zones") {
    std::string readErr;
    const char *zoneOverride = std::getenv("OFFICIAL_ZONES_GEOJSON");
    const std::string zonePath =
        (zoneOverride && *zoneOverride) ? std::string(zoneOverride) : (dataRoot + "/map_official_polygons.geojson");
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/map/compliance-intersections") {
    const char *zoneOverride = std::getenv("OFFICIAL_ZONES_GEOJSON");
    const std::string zonePath =
        (zoneOverride && *zoneOverride) ? std::string(zoneOverride) : (dataRoot + "/map_official_polygons.geojson");
    std::string readErr;
    ...
    if (!raw.empty() && !parsedOk)
      body["geojson_error"] = parseErr;
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/auth/users") {
    const auto session = resolveAuthSession(req, query);
    if (!session) return makeJsonResponse(http::status::unauthorized, json::object{{"error", "unauthorized"}});
    std::string company = query.count("company") ? query.at("company") : session->company;
#if HAS_LIBPQ
    return makeJsonResponse(http::status::ok, json::object{{"users", listCompanyUsersPg(gDatabaseUrl, company)}});
#else
    return makeJsonResponse(http::status::ok, json::object{{"users", json::array()}});
#endif
  }

  if (req.method() == http::verb::post && pathOnly == "/api/auth/users/maintenance") {
    const auto session = resolveAuthSession(req, query);
    if (!session) return makeJsonResponse(http::status::unauthorized, json::object{{"error", "unauthorized"}});
    try {
        ...
        std::string error;
#if HAS_LIBPQ
        if (executeUserMaintenancePg(gDatabaseUrl, payload, audit, error)) {
            return makeJsonResponse(http::status::ok, json::object{{"status", "ok"}, {"audit", audit}});
        }
        return makeJsonResponse(http::status::bad_request, json::object{{"error", error}});
#else
        return makeJsonResponse(http::status::bad_request, json::object{{"error", "postgres support is not compiled"}});
#endif
    } catch (const std::exception &ex) {
        return makeJsonResponse(http::status::bad_request, json::object{{"error", ex.what()}});
    }
  ...
  if (req.method() == http::verb::get && pathOnly == "/api/auth/users/maintenance/audit") {
    const auto session = resolveAuthSession(req, query);
    if (!session) return makeJsonResponse(http::status::unauthorized, json::object{{"error", "unauthorized"}});
    std::string company = query.count("company") ? query.at("company") : session->company;
    int page = query.count("page") ? std::atoi(query.at("page").c_str()) : 1;
    int pageSize = query.count("page_size") ? std::atoi(query.at("page_size").c_str()) : 20;
#if HAS_LIBPQ
    return makeJsonResponse(http::status::ok, json::object{{"logs", listUserMaintenanceAuditPg(gDatabaseUrl, company, page, pageSize)}});
#endif
  }
