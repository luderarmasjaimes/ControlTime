...
export function getBiometricOvalVideoMetrics(box, vw, vh) {
    if (vw < 32 || vh < 32) {
        return null
    }
 ...
 */
export function renderOvalMaskedFrameToCanvas(video, ovalMetrics, tw, th) {
...
export function buildVerifyFrameJpegBase64FromVideo(video, ovalMetrics, tw, th, jpegQuality) {
 ...
 */
export function buildFullFrameJpegBase64FromVideo(video, tw, th, jpegQuality) {
    const canvas = document.createElement('canvas')
    canvas.width = tw
    canvas.height = th
    const ctx = canvas.getContext('2d')
...
export function computeBiometricOvalLayout(box, vw, vh) {
    if (FACIAL_STRICT_OVAL_MODE) {
        return {
...
export function mapOvalLayoutVideoToStage(layout, vw, vh, stageW, stageH) {
 ...
 */
export function frameToTemplate(videoElement, faceBox) {
 ...
 */
export function frameToJpegBase64(videoElement, faceBox, jpegQuality = FACE_SNAPSHOT_JPEG_QUALITY) {
