...
function setStoredUsers(users: any[]): void {
  try {
 ...
 */
function createSeedUsers(company: string): any[] {
 ...
 */
export async function bootstrapUsers(company?: string | null): Promise<void> {
  if (!company) return;
  ...
  try {
    ...
    if (users.length > 0) {
      ...
      setStoredUsers([...existing, ...users.map((u: any) => ({
        ...u,
    ...
    log.debug('Failed to fetch users from backend, using seed data:', (err as Error).message);
  }
 ...
 */
export async function refreshUsers(company?: string | null): Promise<boolean> {
  ...
  try {
    ...
    log.debug('Failed to refresh users:', (err as Error).message);
  }
 ...
 */
export function ensureCompanyUsers(company?: string | null): void {
  if (!company) return;
