...
import { Activity, ListChecks, MapPin, Hash, Plug, Unplug } from 'lucide-react';
...
function slugFromTypeName(name: unknown, unit: unknown): string {
...
function SensorInspector({ element, onUpdate }: SensorInspectorProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError((e as Error)?.message || 'No se pudo cargar el catálogo de sensores');
  ...
  const togglePick = (id: number | string) => {
    setPicked((prev) => {
  ...
  const applyToDocument = () => {
    ...
    const buildProps = (sensorRow: any) => {
  ...
  const isConnected = element.props?.connected !== false;
  ...
  return (
    <div className="inspector-form">
      ...
      <button
        ...
        onClick={() => onUpdate({ props: { ...element.props, connected: !isConnected } })}
        title={
          isConnected
            ? 'Congelar este sensor en su último valor — deja de consultar la base de datos'
            : 'Reanudar el sondeo en vivo de este sensor contra la base de datos'
        }
      >
        {isConnected ? <Unplug size={14} /> : <Plug size={14} />}
        {isConnected ? 'Desconectar de la base de datos' : 'Conectar a la base de datos'}
      </button>
...
export default memo(SensorInspector);
