...
function toIsoLocalDateTime(dateStr: string, endOfDay = false): string {
  if (!dateStr) return '';
  return `${dateStr}T${endOfDay ? '23:59:59' : '00:00:00'}-05:00`;
}

interface FormulaAnalysisModalProps {
  onClose: () => void;
}

function FormulaAnalysisModal({ onClose }: FormulaAnalysisModalProps) {
...
export default memo(FormulaAnalysisModal);
