...
function formatKpiValue(value: unknown, decimals = 2): string {
...
function complianceState(kpi: any): Compliance {
...
function donutMeta(kpi: any): { arcRatio: number; label: string; raw: number } | null {
 ...
 */
function augmentFlatSeriesForDisplay(series: SeriesPoint[], kpi: any): { series: SeriesPoint[]; illustrated: boolean } {
...
function KpiTrendLine({ series, stroke }: { series: SeriesPoint[]; stroke: string }) {
...
function KpiDonutRing({ arcRatio, trackColor, arcColor, centerLabel }: KpiDonutRingProps) {
  const size = 44;
  const cx = size / 2;
  const cy = size / 2;
  const r = 15;
...
interface KpiTrendVizProps {
  mode: string;
  displaySeries: SeriesPoint[];
  illustrated: boolean;
  kpi: any;
...
function KpiTrendViz({ mode, displaySeries, illustrated, kpi, compliance, accentColor }: KpiTrendVizProps) {
  if (mode === 'none') {
    return null;
  }
...
interface MiningKpiWidgetProps {
  kpiCode?: string;
  title?: string;
  trendViz?: string;
  width?: number | string;
  height?: number | string;
  /** Ya NO controla la cadencia real (ver useSharedPoll más abajo: todos los
   * MiningKpiWidget comparten un único timer de 30s) -- se conserva en el
   * tipo solo por compatibilidad con documentos guardados que puedan traer
   * este campo; nunca se expuso en ninguna UI de configuración. */
  refreshMs?: number;
  /** Botón "Conectar/Desconectar de la base de datos" del inspector — en
   * `false` el widget deja de sondear el servidor por completo y muestra el
   * último valor ya cargado como congelado, no un dato en vivo. */
  connected?: boolean;
}

function MiningKpiWidget({
  kpiCode,
  title,
  trendViz,
  width = 200,
  height = 120,
  connected = true,
}: MiningKpiWidgetProps) {
  const [error, setError] = useState('');
  const [kpi, setKpi] = useState<any>(null);
  const [points, setPoints] = useState<any[]>([]);

  const vizMode = TREND_VIZ_MODES.has(trendViz || '') ? (trendViz as string) : 'spark_bars';

  // Lista completa de KPIs, compartida entre TODOS los MiningKpiWidget del
  // documento -- un solo timer/pedido HTTP en vez de uno por widget (ver
  // useSharedPoll.ts; antes N bloques KPI en una página = N pedidos
  // idénticos cada 5-30s, pedido explícito 2026-09-11). `key=null` mientras
  // está "Desconectado" saca a este widget del pool sin afectar a los demás.
  const listKey = connected ? 'miningKpis' : null;
  const {
    data: kpiRows,
    loading: listLoading,
    error: listError,
    refetch: refetchList,
  } = useSharedPoll(listKey, () => fetchMiningKpis(), 30000);

  // El historial de puntos SÍ es por-KPI, pero varios widgets que citen el
  // mismo `kpiCode` (común en un informe) ahora también comparten ese pedido
  // en vez de duplicarlo.
  const resolvedCode: string | undefined = kpiRows?.find(
    (r: any) => String(r.code || '') === String(kpiCode || ''),
  )?.code;
  const pointsKey = connected && resolvedCode ? `miningKpiPoints:${resolvedCode}:7` : null;
  const { data: pointsData, refetch: refetchPoints } = useSharedPoll(
    pointsKey,
    () => fetchMiningKpiPoints(resolvedCode as string, 7),
    30000,
  );

  useEffect(() => {
    const onSynced = () => { refetchList(); refetchPoints(); };
  ...
  }, [refetchList, refetchPoints]);

  // Desconectado: no tocar `kpi`/`points`/`error` -- quedan congelados en
  // pantalla con su último valor (ver badge "Desconectado" en el render).
  useEffect(() => {
    if (!connected) return;
    if (listError) {
      setError((listError as Error)?.message || 'No se pudo cargar KPI');
      setKpi(null);
      return;
    }
    if (!kpiRows) return;
    const found = kpiRows.find((r: any) => String(r.code || '') === String(kpiCode || ''));
    if (found) {
      setKpi(found);
      setError('');
    } else {
  ...
  }, [kpiRows, listError, connected, kpiCode]);

  useEffect(() => {
    if (!connected || !pointsData) return;
    setPoints(pointsData);
  }, [pointsData, connected]);

  const loading = connected && listLoading;
  ...
  const sparkSeries = useMemo<SeriesPoint[]>(() => {
    const raw = Array.isArray(points) ? points : [];
    const mapped: SeriesPoint[] = raw.slice(-24).map((p) => ({
    ...
    <div className="report-kpi-card" style={{ width, height }}>
      <div className="report-kpi-head">
        ...
        {connected ? (
          ...
          <span
            className="report-kpi-pill"
            title="Desconectado de la base de datos — valor congelado, no se actualiza"
...
// Mismo razonamiento que SensorWidget.tsx: props derivadas de element.props
// (referencia estable para elementos KPI hermanos sin editar), estado de
// telemetría en useSharedPoll (fuera de React) no afectado por saltarse un
// render del padre.
export default memo(MiningKpiWidget);
