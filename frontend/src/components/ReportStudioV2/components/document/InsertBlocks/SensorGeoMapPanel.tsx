...
function SensorGeoMapPanel({ points, skippedCount, onReady }: SensorGeoMapPanelProps) {
  ...
  useEffect(() => {
    ...
    // Leaflet mide el tamaño de su contenedor UNA sola vez, al crear el mapa
    // (`L.map()` de arriba) -- si el bloque después se agranda por CSS (el
    // usuario arrastra la esquina del bloque en PageCanvas.tsx), el `<div>`
    // interior de Leaflet sí crece (width/height:100% ya lo sigue), pero los
    // tiles/el viewport internos de Leaflet quedan con el tamaño viejo
    // cacheado -- el resto queda gris hasta que algo llame
    // `map.invalidateSize()`. `animate: false` por el mismo motivo que las
    // demás animaciones desactivadas arriba (zoomAnimation/fadeAnimation):
    // debe quedar síncrono para no competir con `map.remove()` del cleanup
    // de abajo si el bloque se desmonta a mitad de una animación.
