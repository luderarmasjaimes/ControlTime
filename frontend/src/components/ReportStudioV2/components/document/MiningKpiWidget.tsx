...
function MiningKpiWidget({
  ...
  useEffect(() => {
    if (!connected) {
      // Desconectado: no sondear el servidor. El último `kpi`/`points` ya
      // cargados quedan congelados en pantalla (ver badge "Desconectado" abajo).
      setLoading(false);
      return undefined;
    }
    let cancelled = false;
    const load = async () => {
      try {
        ...
        if (cancelled) return;
        const found = rows.find((r: any) => String(r.code || '') === String(kpiCode || ''));
        if (found) {
          setKpi(found);
          setError('');
          try {
        ...
        if (!cancelled) {
          setError((err as Error)?.message || 'No se pudo cargar KPI');
          setKpi(null);
        }
      } finally {
        if (!cancelled) {
  ...
  }, [kpiCode, refreshMs, syncNonce, connected]);
...
// (referencia estable para elementos KPI hermanos sin editar), polling propio
// vía useEffect+setInterval no afectado por saltarse un render del padre.
