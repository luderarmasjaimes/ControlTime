...
const MINING_REPLACEMENTS: [RegExp, string][] = [
...
function applyMiningVocab(text: string): string {
  let result = text;
    ...
    <div className={`vd-panel ${isListening ? 'vd-panel--active' : ''}`}>
      ...
      {showSettings && (
        <div className="vd-settings">
          <label>Idioma:
            <select value={language} disabled onChange={() => {}}>
...
export default memo(VoiceDictation);
