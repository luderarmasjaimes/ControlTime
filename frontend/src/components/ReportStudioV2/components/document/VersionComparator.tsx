...
import {
  ArrowLeftRight, Eye, FileText, Diff,
} from 'lucide-react';
import type { VersionSnapshot } from './VersionHistory';
...
function diffTexts(oldText: string, newText: string): TextDiffEntry[] {
  const oldLines = (oldText || '').split('\n');
  const newLines = (newText || '').split('\n');
  const maxLen = Math.max(oldLines.length, newLines.length);
  const diffs: TextDiffEntry[] = [];
...
function diffElements(oldEls: any[] | undefined, newEls: any[] | undefined): ElementDiffEntry[] {
  const changes: ElementDiffEntry[] = [];
...
function PageDiffView({ leftPage, rightPage, pageNum }: PageDiffViewProps) {
  const leftEls: any[] = leftPage?.elements || [];
  const rightEls: any[] = rightPage?.elements || [];
    ...
    <div className="vc-page-diff">
      ...
      {changes.length > 0 && (
        <div className="vc-changes-summary">
          {changes.map((ch, i) => (
            <div key={i} className="vc-change-item" style={{ '--ch-color': colorForType(ch.type) } as React.CSSProperties}>
...
function VersionComparator({ snapshots = [], currentDoc, onClose }: VersionComparatorProps) {
...
export default memo(VersionComparator);
