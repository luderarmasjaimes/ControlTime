...
interface InclinometerChartsProps {
...
const InclinometerCharts = ({ xRange = [-40, 40], yRange = [0, 40], azimuthAngle = 0, installationAngle = 0 }: InclinometerChartsProps) => {
    const chartRefX = useRef<HTMLDivElement | null>(null);
    const chartRefY = useRef<HTMLDivElement | null>(null);
    const charts = useRef<{ x: echarts.ECharts | null; y: echarts.ECharts | null }>({ x: null, y: null });

    const generateRotatedData = (index: number) => {
        let currentX = 0;
        let currentY = 0;
...
export default memo(InclinometerCharts);
