 ...
 */
export default function TerritorialCompliancePanel({
    siteLabel,
    totals,
    showExternalWms,
    wmsStatus,
    ...
    const intersections = Array.isArray(geoCompliance?.intersections) ? geoCompliance.intersections : [];
    ...
    const criticalGeo = intersections.some(
        (row) =>
            ...
            row.polygons?.some((p) => String(p.severity || '').toLowerCase() === 'high'),
    );
    if (criticalGeo) {
        level = 'red';
