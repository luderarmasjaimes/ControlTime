...
interface DetailedMapProps {
  onCaptureMap?: (dataUrl: string) => void;
  captureButtonPlacement?: 'top' | 'bottom';
...
const DetailedMap = ({
    ...
    const tileJsonCacheRef = useRef<Record<string, any>>({});
    ...
    const [currentTileset, setCurrentTileset] = useState<string | null>(null);
    ...
    // Mismo origen que la SPA (proxificado por nginx/vite hacia el servicio
    // tileserver) — antes hardcodeado a http://localhost:8000, lo que rompía
    // cualquier deploy no-local y violaba connect-src 'self' del CSP.
    ...
    const loadTileset = async (tileset: any) => {
        if (!map.current) return;
        ...
        try {
            ...
            setStatus({ kind: 'error', text: `No se pudo cargar la capa: ${(err as Error).message}` });
        }
    };
    ...
    const captureMap = async () => {
        ...
        try {
            ...
            const mapContainerEl = map.current._container;
            const leafletCanvas = mapContainerEl?.querySelector('canvas');
