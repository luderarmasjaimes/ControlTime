...
import { type Session, updateSessionAvatar } from '../../auth/authStorage'
...
import { fetchMyAvatarHd } from '../../auth/authApi'
import { AvatarBodyTemplatePicker } from './AvatarBodyTemplatePicker'
import { AvatarReEnrollModal } from './AvatarReEnrollModal'
import { ensureCompanyUsers } from '../ReportStudioV2/lib/userBootstrap';
import { log } from '../../lib/logger';
import { ConfirmActionHost } from '../UI/ConfirmActionDialog';
import { motion, AnimatePresence } from 'framer-motion'
import AnimatedButton from '../../components/UI/AnimatedButton'
import ViewErrorBoundary from '../ViewErrorBoundary';
import AzimuthCompass from '../../components/Special/AzimuthCompass'
import UserMaintenanceModal from '../ReportStudioV2/components/modals/UserMaintenanceModal';
import AuditCenter from '../Auth/AuditCenter';
...
interface DashboardAppProps {
    ...
    azimuthAngle: number
    setAzimuthAngle: (v: number) => void
    installationAngle: number
    setInstallationAngle: (v: number) => void
    /** Vista pesada de la pestaña activa -- decidida e importada en App.tsx,
     * NavBar solo la ubica en el slot correcto del chrome. */
...
const NavBar = ({
    ...
    setAzimuthAngle,
    installationAngle,
    setInstallationAngle,
    content,
}: DashboardAppProps) => {
    const { t, language, countryIso2 } = useI18n()
    // Chip "En línea"/"Sin conexión" de la barra superior (pedido explícito
    // 2026-09-08) -- antes era texto fijo, sin relación con la conectividad
    // real. connectivityMonitor.ts ya existía (heartbeat real contra
    // /api/health, con histéresis para no parpadear) para el módulo de
    // mapas; se reusa acá en vez de armar un segundo detector de
    // conectividad independiente. DEGRADADO cuenta como "en línea" (el
    // backend SÍ responde, solo más lento) -- lo que le importa al usuario
    // acá es si hay conexión real o no, no la calidad del enlace.
    const { state: connectivityState } = useConnectivity()
    const isBackendOnline = connectivityState !== 'OFFLINE'
    ...
    // Rediseño de avatar (2026-09-20, ADR-203): "cambiar de vestimenta"
    // recompone la miniatura en el backend, pero `session` es una prop que
    // este componente no controla (vive en App.tsx) -- un override local
    // permite reflejar el resultado en la cabecera al instante, sin esperar
    // a que App.tsx vuelva a leer localStorage (recién en el próximo reload).
    // updateSessionAvatar() igual persiste el valor para esa próxima carga.
    const [avatarOverrideB64, setAvatarOverrideB64] = useState<string | null>(null)
    const [outfitPickerOpen, setOutfitPickerOpen] = useState(false)
    const [reenrollOpen, setReenrollOpen] = useState(false)
    const avatarB64 = avatarOverrideB64 || normalizeSessionAvatarBase64(session)
    ...
    // Compartido por "cambiar vestimenta" y "actualizar mi foto": ambos
    // reescriben avatars_hd/{userId}.png en el backend, así que el HD
    // cacheado en el cliente queda desactualizado igual y hay que refrescarlo
    // con fetchMyAvatarHd() en vez de derivarlo del lado del cliente.
    const applyNewAvatarCartoon = (avatarCartoonBase64: string, logContext: string) => {
        setAvatarOverrideB64(avatarCartoonBase64)
        updateSessionAvatar(avatarCartoonBase64)
        ...
        fetchMyAvatarHd()
            ...
            .catch((error) => {
                log.warn(`[SESSION_AVATAR] no se pudo refrescar el HD tras ${logContext}`, error)
                setAvatarHdFailed(true)
            })
            .finally(() => setAvatarHdLoading(false))
    }

    const handleOutfitApplied = (result: { avatar_body_template_slug: string; avatar_cartoon_base64: string }) => {
        applyNewAvatarCartoon(result.avatar_cartoon_base64, 'cambiar de vestimenta')
    }

    const handleReEnrollApplied = (avatarCartoonBase64: string) => {
        setReenrollOpen(false)
        applyNewAvatarCartoon(avatarCartoonBase64, 'actualizar la foto')
    }
    ...
    const lockMainScrollViewport = mapCentricTabNames.includes(activeTab) || fullBleedWorkbenchTabNames.includes(activeTab)
    useEffect(() => {
    ...
    const tabs: TabDef[] = [
        ...
        {
            // ADR-187: motor de fórmulas real (sensor_formula_def, conectado a
            // sensors/telemetry_multivariate) -- deliberadamente un ítem de
            // menú distinto del "Formula" de arriba (ese es el editor legado
            // sobre el catálogo de ejemplo mineria_sensores, sin relación con
            // los sensores reales). Vista de nivel superior con TODAS las
            // fórmulas del tenant, para no tener que entrar sensor por sensor.
            name: 'FormulaOverview',
            label: 'Fórmulas de Sensores',
            icon: Calculator,
            glyph: 'FX',
            badge: 'Live',
            tip: t('nav.openModule', { name: 'Fórmulas de Sensores' })
        },
        // {
        //     name: 'Report',
    ...
    const enterpriseGroups: EnterpriseGroup[] = [
        {
            ...
            items: ['UserManagement', 'Permissions', 'AlarmConfig', 'DeviceManagement'],
            navTag: 'ADM',
        },
        // {
        ...
        // },
        {
            ...
            icon: Sigma,
            tip: t('nav.openModule', { name: t('nav.reports') }),
            items: ['Formula', 'FormulaOverview', 'Report', 'Report v2'],
            navTag: 'DOC',
        }
    ]
  ...
  return (
    <div
        ...
        >
                ...
                >
                    ...
                    >
                        ...
                        <div className="mining-command-actions ml-auto flex shrink-0 flex-nowrap items-center justify-end gap-2">
                            <div
                                className={`mining-status-chip hidden sm:flex ${isBackendOnline ? 'mining-status-chip--online' : 'mining-status-chip--offline'}`}
                                title={isBackendOnline ? undefined : 'Sin conexión con el servidor -- reintentando automáticamente'}
                            >
                                {isBackendOnline ? <ShieldCheck size={15} /> : <Unplug size={15} />}
                                <span>{isBackendOnline ? t('nav.online') : t('nav.offline')}</span>
                            </div>
            ...
            {
            // codigo para la alerta de mantenimiento de usuarios tras iniciar sesion desactivado
            }
            ...
            {avatarPreviewOpen && headerAvatarDataUrl && (
                ...
                >
                    <section
                        ...
                        // Bug real (2026-09-20): el overlay cierra el diálogo con
                        // onMouseDown ante CUALQUIER mousedown que le llegue burbujeando
                        // -- antes solo el <img> de abajo frenaba esa burbuja
                        // individualmente. Los botones del footer ("Cambiar vestimenta",
                        // "Actualizar mi foto") y los paneles que abren (selector de
                        // vestimenta, captura de cámara) no lo hacían: cualquier click
                        // adentro del diálogo -- incluidas las plantillas de vestimenta y
                        // el botón "Capturar/Confirmar" del modal de re-enrolamiento --
                        // burbujeaba hasta el overlay y cerraba TODO el diálogo antes de
                        // que el click llegara a ejecutar su propio handler. Frenar la
                        // burbuja acá, en la sección completa, alcanza para todo lo de
                        // adentro sin tener que repetirlo en cada hijo nuevo.
                    ...
                    >
                        ...
                        <div className="avatar-preview-dialog__scroll">
                        ...
                        {outfitPickerOpen && (
                            <AvatarBodyTemplatePicker
                                onApplied={handleOutfitApplied}
                                onHeadUnavailable={() => {
                                    setOutfitPickerOpen(false)
                        ...
                        {reenrollOpen && (
                            <AvatarReEnrollModal
                                onApplied={handleReEnrollApplied}
                                onClose={() => setReenrollOpen(false)}
                            />
                        )}
                        <div className="avatar-preview-dialog__actions">
                            <button
                                type="button"
                                className="avatar-outfit-toggle"
                                onClick={() => {
                                    setReenrollOpen(false)
                                    setOutfitPickerOpen((open) => !open)
                                }}
                            >
                            ...
                            <button
                                ...
                                onClick={() => {
                                    setOutfitPickerOpen(false)
                                    setReenrollOpen((open) => !open)
                                }}
                            >
                                {t('avatar.updatePhoto')}
                            </button>
                        </div>
