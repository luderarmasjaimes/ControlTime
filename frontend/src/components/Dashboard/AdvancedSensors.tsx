...
interface AdvancedSensorsProps {
    telemetryTenantId?: string;
}

const AdvancedSensors = ({ telemetryTenantId = defaultScope.telemetryTenantId }: AdvancedSensorsProps) => {
    const [data, setData] = useState<{ categories: any[]; sensor_types: any[]; sensors: any[]; history: any[] }>({ categories: [], sensor_types: [], sensors: [], history: [] });
    ...
    useEffect(() => {
        ...
        setSelectedCategoryId((prev) => (prev != null && catIds.includes(prev) ? prev : catIds[0]));
    ...
    // Serie del sensor seleccionado + metadatos, compartida por los widgets
    // estilo ThingsBoard (gauge radial, tarjetas de agregación) — mismo
    // filtrado/orden que chartOption, extraído para no recorrer el historial
    // dos veces por render.
    const selectedSeries = useMemo(() => {
        ...
        const currentValue = Number(sensor?.current_value);
        return {
            sensor,
            unit: type?.unit || '',
            values,
            currentValue: Number.isFinite(currentValue) ? currentValue : (values[values.length - 1] ?? 0),
        };
    }, [selectedSensorId, data]);

    const gaugeBounds = useMemo(
        () => deriveGaugeBounds(selectedSeries.values, selectedSeries.currentValue),
        [selectedSeries],
    );
                ...
                <>
                    ...
                    <div className="grid min-h-0 flex-1 grid-cols-12 gap-5 pb-4">
                        ...
                        <div className="col-span-12 flex min-h-0 flex-col gap-5 lg:col-span-8">
                            ...
                            {/* Fila de widgets estilo ThingsBoard (ver
                               widgets/SensorWidgets.tsx): gauge radial con
                               zonas del sensor seleccionado, agregación
                               min/avg/max/último de su historial, y donut de
                               distribución por estado del ámbito visible. */}
                            {selectedSeries.sensor && (
                                <div className="grid shrink-0 grid-cols-1 gap-4 sm:grid-cols-3">
                                    <div className="h-[230px] rounded-2xl border border-slate-800 bg-slate-950/50 p-4 backdrop-blur-sm">
                                        <RadialGaugeWidget
                                            value={selectedSeries.currentValue}
                                            unit={selectedSeries.unit}
                                            title="Lectura actual"
                                            min={gaugeBounds.min}
                                            max={gaugeBounds.max}
                                            warn={gaugeBounds.warn}
                                            crit={gaugeBounds.crit}
                                        />
                                    </div>
                                    <div className="h-[230px] rounded-2xl border border-slate-800 bg-slate-950/50 p-4 backdrop-blur-sm">
                                        ...
                                        <div className="h-[calc(100%-18px)]">
                                            <AggregationCards values={selectedSeries.values} unit={selectedSeries.unit} />
                                        </div>
                                    </div>
                                    <div className="h-[230px] rounded-2xl border border-slate-800 bg-slate-950/50 p-4 backdrop-blur-sm">
                                        <StatusDonutWidget sensors={filteredSensors} title="Estado de la categoría" />
                                    </div>
                                </div>
                            )}
...
// telemetryTenantId es el único prop (string primitivo, estable salvo
// cambio real de tenant) — memo evita el re-render forzado por los otros
// 21 useState de src/App.tsx (sliders de otras pestañas, menús, etc.) que
// no afectan a este componente.
export default memo(AdvancedSensors);
