...
import {
  Activity, Wifi, WifiOff, Database, Zap, TrendingUp, TrendingDown,
  RefreshCw, AlertTriangle, Clock, BarChart3, Layers, Server,
  ArrowDown, ArrowUp, Minus, Radio, Signal,
} from 'lucide-react';
...
function MiniSparkSVG({ data, color, width = 60, height = 20 }) {
...
function StreamStatus({ label, icon: Icon, color, msgPerSec, latencyMs, status }) {
  return (
    <div className="td-stream" style={{ '--stream-color': color }}>
      <div className="td-stream-header">
        ...
        <span className={`td-stream-status td-stream-status--${status}`}>
          {status === 'connected' ? '●' : status === 'degraded' ? '◐' : '○'} {status}
        </span>
      </div>
...
function DatabaseStatus({ label, icon: Icon, color, queryMs, connections, storage, status }) {
  return (
    <div className="td-db" style={{ '--db-color': color }}>
...
export default function TelemetryDashboard({ telemetryTenantId }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const sc = { ...sensorCounts };
      const zc = { ...zoneCounts };
      batch.forEach(e => {
        sc[e.type] = (sc[e.type] || 0) + 1;
        zc[e.zone] = (zc[e.zone] || 0) + 1;
      });
      setSensorCounts(sc);
      setZoneCounts(zc);
    }, 1000);
  ...
  return (
    <div className="td-panel">
      <div className="td-header">
        ...
        <span className="td-title">Telemetría Dual-Stream</span>
