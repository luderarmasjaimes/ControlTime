...
export const MINING_LOCATIONS: Record<string, MiningLocation> = {
...
let remoteLocations: Record<string, MiningLocation> | null = null;

export async function initMiningLocations(): Promise<void> {
  try {
    const res = await fetch('/api/config/mining-locations');
    if (res.ok) {
      const data = await res.json();
      remoteLocations = data.reduce((acc: Record<string, MiningLocation>, loc: any) => {
...
export function getMiningLocation(companyName: string | undefined | null): MiningLocation | null {
