...
**Versión:** v36.1 — sincronizada con el SOW Maestro v3.0 (SOW_Maestro_AURIXA_2026_v4) y el Plan de tareas v36
...
**Adelanto técnico v36.1.** Los componentes de mayor riesgo ya están construidos: ingesta durable de 10.000 sensores sin pérdida de datos (cola Redpanda + escritura por lotes), réplica de lectura para alta disponibilidad, pooling de conexiones (10k clientes), compresión columnar y archivado automático de datos > 1 año (tier frío). Detalle y pedido de aprobación en `Resumen_Sprints_Entregables_Gerencia_TI_v36.md`.
...
- **Pérdida de telemetría en ráfagas y saturación de la base (NUEVO) → mitigados por diseño** con cola durable Redpanda (commit por lote), PgBouncer (pooling) y réplica de lectura.
- **Costo de almacenamiento a largo plazo (NUEVO) → mitigado** con tier frío automático a MinIO/S3 (Parquet ZSTD) para datos > 1 año.
