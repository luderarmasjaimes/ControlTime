...
**Status**: parcial, ampliado 2026-07-09 (gestión de dispositivos + motor de alarmas cerrados; adaptadores de protocolo siguen diferidos por diseño). El propio texto de este ADR scopea "v0.1: los protocolos en uso hoy + motor de fórmulas + ingesta básica" — Modbus/OPC-UA/MQTT/CoAP quedan explícitamente en "Futuro (por demanda)", no son un hueco de v0.1. De las 5 capacidades que este ADR pide, las 2 restantes que SÍ estaban genuinamente faltantes (gestión de dispositivos, alarmas) se implementaron en esta sesión:
