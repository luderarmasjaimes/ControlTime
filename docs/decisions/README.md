...
> **Actualización 2026-09-23: auditoría integral, cierre de ADR y plan maestro
> (ADR-210, 211, 212).** Log verificado sin huecos ni duplicados (ADR-000 a
> ADR-212). Se cerraron por evidencia los estados de ADR-085, 086, 088, 109,
> 110 (→ deferred, ADR-178), 116 y 150 (Fase A implemented; B/C deferred). Se
> detectaron y documentaron 3 hallazgos operativos: migraciones 115/116 sin
> aplicar en la BD viva aunque el backend ya las usa; runner de migraciones
> bloqueado por dos scripts con prefijo 81 (ADR-211); y 400 cambios sin commit
> (ADR-171 a 209 y migraciones 93-116 fuera de git). Los pendientes de todos los
> ADR abiertos (035, 033, 117, 131, 156-159, 161, 163, 164, 168, 169, 170, 176,
> 188, 189, 194, 196, 202, 203, 207, 208) quedan gobernados por el registro
> G-1…G-17 de **ADR-212**. Avance: 72,5% oficial · 73,3% auditado · ≈47 tareas
> de backlog fino de salida a producción.
...
> 151 y 152 — no existen esos dos números).** *(Hueco cerrado 2026-09-11 —
> ver ADR-151/152, archivos mínimos que retiran formalmente ambos números
> sin renumerar nada ya citado; el log queda otra vez sin huecos reales,
> 000-168.)* A pedido explícito del developer
...
>    correctamente. *(Corregido 2026-09-11, a pedido explícito del developer:
>    las 13 citas erróneas en los 4 archivos se cambiaron de "ADR-143" a
>    "ADR-145" — corrección mecánica de referencia, no una decisión nueva, no
>    requiere bloque de actualización propio.)*
...
| 075 | `internacionalizacion-pais-idioma-acceso` | ✅ implemented · probado · desplegado; brecha de cobertura de claves pendiente (act. 2026-09-12) | País→idioma ES/EN/FR/PT-BR, fiscal/prefijo y diálogos propios; imágenes activas y smoke aprobado. Actualización 2026-09-12: confirma que la expansión reciente (Home/NavBar) sigue en el alcance de 4 idiomas prometido, pero encontró que `TranslationKey` ya no fuerza (vía `(string & {})`) la misma superficie de claves entre idiomas como el ADR afirma — hasta 154 claves de diferencia en/pt con fallback silencioso a español. Pendiente: completar claves + restringir el tipo + test de cobertura. |
...
| 175 | `navbar-extraccion-shell-autenticado` | ✅ implemented, verificado conforme (formalizado 2026-09-12) | `NavBar.tsx` extrae ~1600 líneas de cabecera/navegación de `App.tsx` (338 líneas resultantes) a un componente propio. Verificado explícitamente sin regresión: diseño de dos carriles/flechas de ADR-040 intacto, nomenclatura llana de ADR-042 intacta, cero uso de `localStorage` (ADR-132), `UserMaintenanceModal` sigue montado por import estático con su CSS autocontenida (ADR-095). Refactor de código, no cambia ninguna decisión. |
| 177 | `decomiso-autoasignacion-rol-autoregistro-publico` | ✅ implemented, verificado por build real y CTest (2026-09-12) | Decomisiona por completo (no solo mitiga) el vector de ADR-134/135: `POST /api/auth/register` ya no acepta ningún rol del cliente (ni el campo `role` ni el heurístico de username `admin`/`admin_*`, `resolveRoleForUsername()` retirada); toda cuenta autoregistrada nace `"operator"`. Decisión explícita: esa capacidad era solo un atajo de pruebas de desarrollo. Build de verificación (`Dockerfile.verify`) compiló limpio y CTest pasó 100%. |
| 178 | `operaciones-campo-fuera-de-alcance-implementacion-futura-independiente` | ✅ accepted (decisión de alcance, 2026-09-12) | Operaciones de Campo (SPEC-022, ver ADR-110) queda **fuera del alcance de este proyecto** — no es un riesgo, pendiente ni ítem de seguimiento de la plataforma minera Beemetry/AURIXA actual. Se conserva `specs/022-*` como referencia técnica para una futura iniciativa independiente; excluida de `scripts/project-status-metrics.ps1`. No reabre el contenido técnico de ADR-110. |
| 191 | `auth-users-is-active-generated-column` | ✅ implemented, verificado en vivo (2026-09-16) | Reporte real: login de `09637521`/Alpayana pasaba (`200 OK`, tokens válidos) pero moría en el primer refresh (`invalid_or_expired_refresh_token`), cerrando sesión en silencio sin código de error. Causa: `loginPasswordPg` valida `account_status` y `findValidRefreshTokenUserPg` valida `is_active` — dos columnas independientes que ningún código sincronizaba (el único endpoint de admin, `POST /api/auth/users/maintenance`, solo escribe `account_status`; `scripts/delete-company-users-docker.ps1` solo escribía `is_active`, al margen de ese endpoint). Fix estructural: `is_active` pasa a columna `GENERATED ALWAYS AS (...) STORED` derivada de `account_status` (`db_scripts/101`) — autocura el drift existente (3 cuentas corregidas por el propio `ALTER`) y Postgres rechaza de ahora en más cualquier `UPDATE` directo a `is_active`, así que la clase de bug no puede repetirse en silencio. Script de ops corregido para escribir `account_status` como el resto de la app. |

**Ámbito `plataforma`: 52/56 implemented/verified, 1 partial, 1 proposed, 2 pendiente verificación E2E** (recalculado 2026-09-02 tras el cierre E2E de ADR-130, +2 el 2026-09-04 con ADR-153/154, +1 el 2026-09-07 con ADR-161, +1 el 2026-09-07 con ADR-162 (pendiente prueba con cámara real), +1 el 2026-09-12 con ADR-175, +2 el 2026-09-12 con ADR-177/178, +1 el 2026-09-16 con ADR-191; ADR-033 sigue partial deliberadamente, ADR-035 sigue proposed a la espera de decisión de negocio — ver ADRs individuales).
...
| 110 | `operaciones-campo-offline-integracion-erp` | 📋 proposed — **fuera de alcance de este proyecto (ADR-178, 2026-09-12)** | Separa fuentes autoritativas ERP/Beemetry, cliente offline con outbox y artefactos auditables; arquitectura de referencia para una futura iniciativa independiente, ya no un pendiente de este proyecto (ver ADR-178). |
...
| 140 | `alertas-umbral-cache-tasa-debounce-sensor` | ✅ implemented, verificado E2E en vivo (2026-09-02); cierre de SPEC-016 completado 2026-09-13 | Reconcilia SPEC-016 con el motor de alarmas real de ADR-034: cache de reglas TTL 60s, condición por tasa de cambio, debounce por ventana (complementa el índice único parcial existente), `PUT` de reglas, paginación del historial, y evaluación EN TIEMPO REAL vía hook en `TelemetryIngestor::copyBatch()` (179ms medido en vivo, reemplaza la dependencia exclusiva del polling de 10s para reglas de `sensor_id`). Canal SSE que SPEC-016 pedía, no construido a propósito — el push real ya es WebSocket. **2026-09-13**: cerrado el DoD pendiente de SPEC-016 — T1-T12 (11/12, bookkeeping de la decisión ya tomada arriba) y Art. 5 de la Constitución, que tenía un gap real (sin métricas de salud/throughput pese a estar en producción desde ADR-034): agregado `mining_iot::AlarmEngineStats` con 5 contadores/gauges reales, expuestos en `/api/metrics` como `beemetry_alarm_engine_*`, verificado desplegado. Al intentar el demo en vivo de gate R5 se encontró un bug real de rendimiento en el propio camino de polling — ver ADR-186; el intento de crear la regla de prueba fue bloqueado correctamente por RBAC (rol `viewer`), demo sigue pendiente por ese motivo, no por código. |
| 186 | `bug-real-timescaledb-chunk-scan-sin-cota-tiempo` | ✅ implemented, verificado en vivo antes/después (2026-09-13) | La consulta de "último valor" del evaluador de alarmas (`evaluateRulesOnce`) no tenía cota de tiempo — con `chunk_time_interval=1h` (ADR-131), `telemetry_fact` ya acumuló ~10.900 chunks, y esa consulta no completaba ni con `statement_timeout` de 10s. El mismo patrón exacto apareció en `GET /api/mining/telemetry/summary` (dashboard general, confirmado colgado 45s+ reales) y en `simulation_status_routes.cpp`. Fix real: acotar `captured_at` a una ventana reciente (15 min en el evaluador, 24h en los paneles de dashboard) en los 3 sitios — permite constraint exclusion, toca solo 1-2 chunks. Medido: `handleGetTelemetrySummary` de 45s+ (no completaba) a 1179ms real contra el stack desplegado. Corrige la premisa de un comentario preexistente que asumía el riesgo ligado al volumen de filas/s en vez de a la cantidad de chunks acumulados con el tiempo transcurrido. |
| 187 | `administracion-sensores-motor-formulas-tiempo-real` | ✅ implemented, verificado E2E en vivo (2026-09-14) | Pantalla `SensorManagementView.tsx` completa: registro/edición de parámetros de `sensors` (`PUT /api/mining/devices/{id}`), prueba de conexión (`POST .../rotate-key` + envío real con `credentials:'omit'` para esquivar el gate CSRF de ADR-082 en una ruta autenticada solo por `X-Device-Key`), enlace a `AlarmConfigView` (restaurado en el nav) para umbrales simples, y el **motor de fórmulas completo**: activa el esquema dormido de `db_scripts/17` con dos tablas nuevas (`sensor_input_parameter_value`, `sensor_formula_def`, `db_scripts/93`), `tinyexpr` vendoreado (zlib) para evaluar expresiones sobre `value` (telemetría real) + parámetros numéricos por sensor, hilo evaluador propio (poller 10s, no engancha el hot path de ingesta) que escribe a `telemetry_multivariate` con `status` (ok/warning/error) nuevo. Bug real preexistente de ADR-034 encontrado y corregido en vivo: `TelemetryIngestor` cacheaba sensores una sola vez en `start()` — `refreshSensorCache()` nuevo, protegido por `shared_mutex`, llamado tras cada registro. Investigado y descartado reusar `formula_routes.cpp`/`formula_engine/` (legado, catálogo `mineria_sensores` sin FK a `sensors` real). Verificado con datos reales: `value - offset` con `offset=5` sobre una lectura real de 62.7 → resultado 57.7 escrito correctamente; segunda fórmula con `error_high=50` → `status='error'` correcto; UI mostrando ambos badges de color. |
| 188 | `reconstruccion-tab-calculo-motor-real` | ✅ implemented, gap de autorización por tenant cerrado (2026-09-14, segunda pasada) | Revierte parcialmente la decisión de ADR-187 de no tocar el tab legado "Cálculo": ese editor pedía Empresa/Unidad Minera porque toda su identidad de diagrama (`emp{X}_mina{Y}`) y su base Postgres propia (`formula_db`) eran un catálogo de demostración fabricado, sin relación con `sensors` real — y su "lógica de fórmula" (`rules.expr`) nunca se evaluaba en ningún lado (confirmado por exploración exhaustiva). Nuevo catálogo gobernado `sensor_type_def`/`sensor_type_parameter_def` (`db_scripts/94`), administrable por `formula.edit` (permiso sembrado en `db_scripts/42`, nunca usado hasta ahora); `formula_engine` repuntado a la base real (`pgbouncer`/`sensors_db`, ya no `formula_db`); selección tipo→zona→sensor reusando `GET /api/mining/telemetry/wizard/catalog` (mismo patrón que `ZoneSensorPicker.tsx`); botón "Guardar fórmula" nuevo que persiste contra `POST /api/mining/devices/{id}/formulas` (ADR-187) en vez del campo muerto `rules.expr` — el evaluador real de 10s empieza a calcularla de inmediato. Segunda pasada, mismo día: las rutas de lienzo del sidecar (`/api/state`, `/api/block*`, `/api/connection*`, `/api/rule*`) ahora validan tenant real por `sensor_id` en cada request vía `GET /api/internal/resolve-session` + `sensors.tenant_id`, fail-closed en todo caso ambiguo — verificado en vivo (401 sin sesión, 400 en `diagram_id` de formato legado). |
| 189 | `catalogo-plantillas-formula-calibracion-geotecnica` | ✅ implemented (backend+frontend), pendiente aplicación manual del SQL y verificación contra datos reales | Investigación profunda de la cadena de reglas ThingsBoard legada `PiezometerRC-V2` (78 nodos/100 conexiones) — no es un editor de fórmulas libre, es un catálogo fijo de 23 familias de física de calibración de instrumentos geotécnicos (Geokon, RST, Soil Instruments, Slope Indicator, Casagrande), seleccionadas por atributo `shared_equation`. El motor real (ADR-187) solo admitía una variable de telemetría (`value`, `channel_id=0`) — insuficiente para las 23 familias (todas necesitan Freq+Temp mínimo, 4 necesitan Press). Cierra 4 decisiones del developer: catálogo de plantillas parametrizadas sobre `tinyexpr` (no portar el JS ni sus excepciones hardcodeadas por dispositivo), las 23 familias completas (menos 2 confirmadas muertas en el propio comentario del legado), alarmas multibanda en el sistema real de alarmas (`platform_alarm_rules`, no duplicado), e ingesta multicanal acotada solo a sensores multivariados (`sensor_input_channel_def` nuevo, `POST /api/mining/telemetry/multi`, cero regresión para sensores de 1 canal). Catálogo de 27 plantillas (`db_scripts/95`) con endpoint de aplicación (`POST .../formulas/apply-template`) y UI en `SensorManagementView.tsx`. Compiló limpio contra el stack real (Catch2 100%) y sin errores de TypeScript; pendiente aplicar el script SQL manualmente y verificar numéricamente contra un sensor real antes de usarse en producción. |
| 208 | `crud-zonas-mineras-sensor-zones` | 🟡 implemented, verificado en build real (Catch2 100%); pendiente verificación E2E en vivo contra el tenant Alpayana | Se pidió confirmar si la creación de zonas mineras estaba al 100% para grabar una demo — no lo estaba: `sensor_zones` (`db_scripts/54`) solo se sembraba por SQL (4 cuadrantes automáticos + "Sin zona asignada"), y el backend solo permitía reasignar el `zone_id` de un sensor a una zona YA existente. Se agrega CRUD real: `GET/POST /api/mining/zones` + `PUT/DELETE /api/mining/zones/{id}` (mismo patrón de sesión/permiso `dispositivos.manage` y `PQexecParams` parametrizado que el resto de `device_alarm_routes.cpp`; borrar una zona no bloquea ni exige reasignar a mano — usa el `ON DELETE SET NULL` que la FK ya declaraba). Nuevo panel "Zonas mineras" en `SensorManagementView.tsx` (crear/renombrar inline/eliminar con confirmación). |

**Ámbito `core-iot`: 13/14 implemented, 1 pendiente de verificación E2E en vivo (ADR-189 pendiente de verificación numérica contra datos reales antes de uso en producción; ADR-208 pendiente de demo grabada — ver sus filas).**
...
| 197 | `importacion-sensores-thingsboard-legado` | 🟡 implemented, importación verificada (2,269 sensores/27 tenants); traductor de fórmulas entregado con 0 automáticas (honesto, ver addendum 2026-09-18: extensión tinyexpr iif/max/min/round + reparación de bug ASI en el parser, mismo resultado final confirmado con mayor rigor) | 2,269 sensores reales del ThingsBoard legado importados vía `dblink` a `sensors`, inscritos en 27 tenants reales (18 nuevos autorizados explícitamente). Traductor `scripts/translate_thingsboard_formulas.py` para las 175 fórmulas legadas de rule-chain: 0 traducibles automáticamente con confianza -- son reglas de negocio con estado/strings/fechas, no calibración numérica pura (ver addendum: se probó also un detector de "selector de N vías" con la nueva extensión iif() del motor, 0 candidatos reales tras excluir casos con arrastre de estado). |
| 198 | `motor-formulas-estado-salidas-multiples-alarmas-legado` | 🟡 implemented (motor: prev_value + múltiples salidas + parámetros metadata.*, verificado con build real); análisis de alarmas sin insertar | `sensor_formula_evaluator.cpp` gana `prev_value` (memoria de 1 ciclo) para histéresis/suavizado. Traductor de fórmulas legadas gana múltiples salidas por registro + metadata.* como parámetro de calibración (con chequeo de colisión de ESTADO real, bug encontrado y corregido en verificación) + parseFloat/comentarios/max/min/round: **5→23 traducciones automáticas reales** (de 305 filas totales, 178 needs_manual_review con motivo específico, 114 de ellos por salida de texto real -- intento de soporte de clasificación/texto descartado con evidencia, esa lógica vive en los nodos de alarma, no en las fórmulas). `scripts/translate_thingsboard_alarms.py` clasifica los 104 nodos de alarma legados contra `platform_alarm_rules`: 5/104 son candidatos reales, 99 son plomería de enrutamiento sin equivalente 1:1. |

**Ámbito `datos`: 7/7 implemented.**
...
| 014 | `estado-editor-zustand` | ✅ implemented (2026-07-06); regla dura aclarada 2026-09-12 | Store Zustand único (no Redux); límite claro cliente↔servidor. Actualización 2026-09-12: aclara que un store adicional puramente efímero (nunca contiene `doc`, nunca se persiste, ciclo de vida de una sola interacción) sí está permitido — caso real: `useDragPreviewStore` (ADR-174). |
...
| 022 | `offline-cola-versionada-indexeddb` | ✅ implemented — cierre completo (2026-07-13); CA-2/CA-3 de SPEC-014 cerrados (2026-09-13) | Persistencia offline real (SQLite/WASM en vez de IndexedDB) + resolución de conflicto real: concurrencia optimista server-side (`expected_version`/409), prompt de reconciliación al reconectar, y elección sobrescribir-vs-guardar-como-nuevo al guardar. Más: checkpoint forzado cada 3 min en línea (pedido de negocio adicional). **2026-09-13**: CA-2 ("sin pérdida y sin duplicar") analizado y confirmado equivalente por diseño — un reintento con el mismo `expected_version` no puede duplicar un guardado (el primero ya incrementó `version_number` bajo lock de fila). CA-3 (bug real): la resolución de un conflicto quedaba indistinguible de un autosave normal en el historial — corregido etiquetando la revisión (`offline_conflict_overwrite`/`offline_conflict_kept_as_new`), visible en el historial de versiones del frontend. |
...
| 051 | `copiar-pegar-objetos-lienzo` | ✅ implemented, actualizado 2026-09-11 | Copiar/pegar de bloques. Actualización 2026-09-11: se integra el portapapeles del SISTEMA operativo (pedido explícito, copiar entre informes distintos) — sensor/KPI/gráfico en vivo solo se reconstruye "vivo" si copia y pega la misma sesión y el destino está editable; de lo contrario, nota de texto sin binding. De paso se agrega `documentLocked` (bloquea edición/deshacer/pegado en informes `signed`/`archived`, ADR-018/079). |
...
| 172 | `motor-tablas-excel-formulas-formato-condicional` | ✅ implemented, alcance v1 acotado a propósito (formalizado 2026-09-12) | Motor de hoja de cálculo embebido en `TableBlock.tsx`, formalizado retroactivamente: 7 fórmulas (SUMA/PROMEDIO/CONTAR/MAX/MIN/ABS/REDONDEAR, alias ES/EN, errores tipo Excel, detección de ciclos), formato condicional (8 reglas de umbral + escalas de color 2/3 puntos, sin barras de datos/iconos), formato numérico (general/porcentaje/decimal, sin moneda/fecha), fusión de celdas, autoajuste de columnas, portapapeles compatible Excel/Sheets/Word, gráfico estático desde tabla. Documenta explícitamente lo NO implementado (BUSCARV, SI, referencias absolutas, ordenar/filtrar, congelar paneles) y deja pendiente de decisión si el resultado de una fórmula debe congelarse al firmar (mismo patrón que ADR-012 para sensor/kpi). |
| 173 | `importacion-word-docx-mammoth` | 🟡 implemented, con hallazgos abiertos sin resolver (formalizado 2026-09-12) | Importación de `.docx` (ribbon Datos→Importación) vía `mammoth` + pipeline propio (`docxPageBreaks.ts`: saltos de página, alineación, color/tamaño de fuente, sombreado de tabla) que reutiliza el parser de pegado (`richPaste.ts::parseRichClipboardBlocks`). ADR-139 es solo exportación, esto es alcance nuevo. Documenta 6 soluciones pendientes priorizadas: validar firma ZIP antes de parsear, resumen post-importación de simplificaciones reales, límite de tamaño/progreso para archivos grandes (ver ADR-170), sumar este flujo al alcance del pentest externo (ADR-169) — el HTML de mammoth nunca pasa por `sanitizeRichHtml` (diseño deliberado y razonado, pero sin auditoría de seguridad dedicada todavía —, preservar tachado, y evaluar tablas anidadas. |
| 174 | `capacidades-avanzadas-editor-store-navegacion-diapositivas` | ✅ implemented (formalizado 2026-09-12) | Formaliza historial deshacer/rehacer con agrupación temporal (600ms), autoFlow (auto-colocación sin superponer), linkedGroup (fragmentos de tabla/texto partidos por paginación que se mueven/seleccionan juntos y pueden refusionarse), y pageSetup (reescalado proporcional al cambiar tamaño/orientación de hoja) en `useEditorStore.ts`; más navegación de diapositivas (`SlideThumbnailRail`, `DragPreviewOverlay`+`useDragPreviewStore`, 12 layouts en `slideLayouts.ts`) sobre infraestructura ya decidida (ADR-083/128). Deja pendiente, sin resolver: la paleta de `slideLayouts.ts` no es todavía la identidad visual oficial (ADR-092), y la reorganización de `InsertBlocks/` mezcló mover archivos con cambios funcionales reales (TableBlock 596→2716 líneas) en el mismo commit — señalado para revisión futura, no corregido acá. |
| 176 | `catalogo-pendientes-borrado-mejora-correccion` | 📋 proposed — cataloga, no decide (2026-09-12) | Registra 3 hallazgos menores con evidencia y opciones para decidir después, sin resolver ninguno: `authSessionManager.ts` (renovación proactiva de token, completo pero sin conectar — falta 1 línea de wiring), `pnpm-workspace.yaml` (artefacto de pnpm sin efecto en un proyecto npm, sin trackear en git, seguro de borrar), y 3 utilidades menores sin riesgo (`usePopover.ts`, `useSharedPoll.ts`, `sensorMockData.ts` — esta última con su insignia "DATOS DE PRUEBA" forzada visible, riesgo ya cubierto). |
| 179 | `push-sse-kpis-bug-real-query-y-cliente-frontend` | ✅ implemented, verificado por build real + test automatizado (19/19) + ejecución en vivo de la query SQL contra Postgres real (2026-09-12) | Cierra SPEC-005 T9/T10/T11/T12. `handleLiveKpiSse` nunca había funcionado: pedía columnas `name`/`value`/`tenant_id` inexistentes en `mining_runtime_kpis` (real: `code`/`current_value`, sin `tenant_id`) — devolvía `kpis: []` en cada evento SSE sin ningún error visible desde que se escribió. Corregido y verificado en vivo contra Postgres real. Nuevo cliente `frontend/src/lib/useLiveKpi.ts` (`EventSource` nativo, auth por cookie HttpOnly) integrado en `KpiOperationsView.tsx` (badge "En vivo"/"En vivo (degradado)"). T9 (degradación graceful) y T12 (`proxy_buffering off` en nginx) se confirman ya implementados, solo sin documentar. |
| 181 | `sse-kpis-pool-conexiones-en-vez-de-asio-strands` | ✅ implemented, verificado por build real (CTest 100%), deploy real, y 2 pruebas reales contra el stack completo (PASS, 2026-09-13) | Cierra el riesgo real de escalabilidad detrás de SPEC-005 T19 ("migrar a Boost.Asio async, un strand por conexión, para >200 conexiones simultáneas"). El backend es thread-per-connection, no async — migrar literalmente exigiría rediseñar toda la capa de sockets del servidor. El riesgo real no era la falta de async sino que `handleLiveKpiSse` abría una conexión Postgres cruda dedicada por cliente SSE, retenida por horas — con 200 viewers, 200 conexiones fuera de cualquier pool. Fix: cada tick (~2s) pide un lease de `storage::PgPool::replica()` (fallback `::instance()`) y lo devuelve de inmediato. **T17 (caída real de réplica)**: degradación confirmada con datos reales sin cortes; encontró y corrigió un bug real (`connect_timeout` faltante en la URL de réplica) que hacía la recuperación ~18× más lenta de lo necesario (7,4 min → 24s). **T19 (carga real de 200 conexiones)**: PASS — 200/200 conectadas, sin degradación real del resto del tráfico, ejecutado por el usuario contra el stack real. |
| 182 | `verificacion-export-20-tipos-grafico-bugs-histograma-funnel` | ✅ implemented, verificado por export real (PDF+PPTX, 378 pág./diapositivas) + test de regresión (2026-09-13) | Cierra SPEC-021 T9: exportó y verificó un PDF/PPTX real con los 20 tipos de `SensorMultiChartWidget` contra el stack completo. 12/20 sin hallazgos; 2 bugs reales corregidos: `histogram` mostraba las 10 etiquetas de bucket idénticas "0.0–0.0" para sensores de magnitud pequeña (acelerógrafos en g) por un `.toFixed(1)` fijo — ahora decimales derivados del ancho real del bucket; `pie`/`donut`/`funnel` recortaban el nombre del sensor a mitad de palabra en el layout pequeño de export — ahora truncan con "…" vía `label.overflow`. `sunburst` queda con un hallazgo de solapamiento de etiquetas documentado, sin corregir (alcance acotado). Test de regresión nuevo verificado en ambas direcciones (falla contra el código viejo, pasa contra el corregido). |
| 183 | `benchmark-real-o3-export-pdf-falla-umbral-5s` | ⚠️ measured — criterio O3 real y actualmente NO se cumple (2026-09-13) | Cierra SPEC-007 T15 (la tarea de MEDIR, no el criterio): benchmark real contra 2 informes reales de 56/63 páginas (no fabricados para la prueba) — 29,4s y 28,9s medidos (reloj del servidor), ~5,9× el umbral de 5s de O3. Diagnóstico por lectura de código: cualquier informe >25 páginas (`EXPORT_VIRTUALIZATION_PAGE_THRESHOLD`) activa un modo de export que monta y espera cada página del sidecar Puppeteer una por una (~2s/página/worker, 4 workers en paralelo) — un informe de 50 páginas cae siempre en ese camino lento. No es un fallo funcional (ambos exports terminaron `success`), es un techo de arquitectura real que requiere una decisión de Gerencia/arquitectura para cerrarse (subir el umbral de virtualización, más paralelismo, o rediseño). Justificación verificada contra el código y su historial: el soporte real para exportaciones muy grandes (2104 pág./6300 diagramas, ADR-170) es la causa confirmada de esta arquitectura; el crecimiento del editor Word/PPTX en general NO participa del cuello de botella (solo 3 widgets de sensor usan el gate de espera) — el catálogo de gráficos de sensor (10→20 tipos, SPEC-021) sí aporta al costo marginal por página. Script de benchmark reutilizable: `scripts/benchmark-export-o3.browser.js`. |
| 184 | `optimizacion-real-export-o3-fuentes-locales-domcontentloaded` | ✅ implemented, verificado en vivo antes/después contra 4 informes reales (2026-09-13) | Profundiza ADR-183: instrumentación real con timestamps reveló que el diagnóstico inicial era impreciso — el arranque de Chromium real toma ~200-300ms, no ~3,5-4s; el costo real estaba en `page.goto(waitUntil:'networkidle0')`, una espera redundante con la verificación de "listo" (`waitForReportRender`) que el código ya hacía justo después. 2 fixes reales: fuentes auto-hospedadas (`frontend/public/fonts/`, cada worker es un Chromium sin caché que antes descargaba de Google en cada arranque) y `waitUntil` bajado a `domcontentloaded` en las 4 páginas que abre el sidecar (PDF/DOCX/PPTX). Medido antes→después: 11 pág. 6,3s→4,6s (-28%), 56 pág. 29,4s→26,8s (-9%), 63 pág. 28,9s→27,2s (-6%), 378 pág./1120 gráficos 278s→272s (-2%, sin regresión, 0 fallos de captura) — mejora real en todos los tamaños, documentos grandes protegidos. O3 sigue sin cumplirse (extrapola a ~24s/50 páginas); ver ADR-185 para el cierre de negocio de esta brecha. |
| 185 | `decision-negocio-cierre-o3-export-async-tolerancia-espera` | ✅ accepted (decisión de negocio, 2026-09-13) | Cierra formalmente SPEC-007 T15/O3: Gerencia (Luder Armas) decide aceptar el tiempo de export medido y optimizado (ADR-184) en vez de invertir en cerrar la brecha restante hacia <5s. Razonamiento: el export es asíncrono (job + polling, no bloquea al usuario); ~24-27s de espera de fondo para el caso más pesado (informe grande con gráficos de sensor) es operacionalmente aceptable; las 3 alternativas restantes (subir umbral de virtualización, más paralelismo, rediseño) tienen costo/riesgo real sin caso de negocio hoy — subir el umbral en particular arriesga reproducir el incidente de documentos gigantes que motivó la arquitectura actual (ADR-170). Documenta explícitamente que el valor literal de la SOW (`spec.md` CA-2/O3, <5s) NO se cumple — desviación reconocida y aceptada, no oculta; `spec.md` no se edita, queda como constancia histórica del target contractual. T15 pasa a ☑, SPEC-007 llega a 100% (6/6), R3 sube a 97,1% (68/70). |
| 199 | `importacion-pdf-ocr-avanzado` | ✅ implemented, verificado end-to-end en vivo (build real del backend + imagen Docker real de `ocr_engine` + requests reales, 2026-09-19; ampliado y re-verificado 2026-09-20) | Importación de PDF con OCR avanzado (ribbon Datos→Importación, junto a "Importar Word" de ADR-173): sidecar nuevo `ocr_engine/` (PaddleOCR PP-StructureV2, CPU-only por conflicto de dependencias con TensorFlow -- mismo motivo que separó `avatar_engine`/`silentface_engine`, ADR-141), proxiado a través de `ai_engine` (`/ocr_pdf`). Extracción híbrida por página: texto digital real (PyMuPDF) cuando existe, OCR completo (layout+tabla+figuras) solo en páginas sin texto embebido. Resuelve desde el día 1 dos gaps que ADR-173 había dejado pendientes para .docx: validación de firma de archivo (`%PDF-`) en las 3 capas, y resumen post-importación (páginas digitales vs. OCR, confianza promedio). Verificación en vivo encontró y corrigió un bug real de la librería paddleocr (`PPStructure(lang='es')` tronaba el proceso con `sys.exit` -- el modelo de layout solo soporta en/ch): se separó layout/tabla (lang='en') del reconocimiento de texto real (`PaddleOCR(lang='es')` aparte). Limitación de precisión real y documentada (no un bug): el modelo multilingüe "latin" a veces omite o confunde diacríticos del español (ej. "año"→"ano"). Tope v1: 80 páginas escaneadas por documento (sin job asíncrono todavía). **Ampliación 2026-09-20 (§7)**: spans de estilo real (negrita/cursiva/color/tamaño/fuente), encabezados (esquema real del PDF o tamaño relativo) que alimentan la Tabla de Contenidos del editor sin código nuevo, alineación de párrafo, listas con profundidad, imágenes intercaladas en orden de lectura real, y geometría de página (tamaño/orientación/márgenes) aplicada al documento importado -- reutiliza `TextStyleSpan`/TOC/`setPaperSize`/`setOrientation`/`setPageMargins` ya existentes. 3 bugs reales encontrados y corregidos en la verificación: encabezados numerados confundidos con ítems de lista, alineación falsa "centrado" (comparaba contra el borde de la hoja, no el área de contenido real), profundidad de lista siempre 0 (comparaba contra el propio bloque del ítem). |
| 201 | `xlsx-tablas-pptx-nativo-pdf-indice-real` | ✅ implemented, verificado por build real del backend (Docker/CTest 100%) + typecheck completo del frontend (0 errores) + pruebas unitarias directas de cada builder nuevo; sin E2E en vivo contra Chromium todavía (2026-09-20) | Lleva PPTX y XLSX al mismo estándar nativo que ADR-139/reportDocxBuilder.js. Helpers puros extraídos a `reportSharedHelpers.js` (compartidos por los 4 formatos); PPTX reconstruido diapositiva por diapositiva desde el JSON (`reportPptxBuilder.js`, retira el overlay DOM viejo), con fix de un efecto colateral real sobre `/render-video` (ADR-084) que asumía una imagen de fondo por diapositiva. XLSX nuevo (`reportXlsxBuilder.js`, `exceljs`): exporta únicamente las tablas ya insertadas, una hoja real por tabla + índice con hipervínculos reales. PDF gana marcadores de navegación reales (árbol armado a mano con la API de bajo nivel de pdf-lib, sin equivalente de alto nivel) y enlaces internos reales sobre el bloque TOC visible, más una opción de exportarlo sin contraseña gateada por un permission code nuevo (`informes.export_sin_clave`, sembrado a admin/manager). |
| 204 | `fidelidad-maxima-docx-flujo-listas-nativas-tachado-hipervinculos` | ✅ implemented, verificado con exports/imports reales (Node directo + navegador en vivo, inspección de XML OOXML), auditoría dedicada de PDF sin brechas encontradas, y verificación final con documento sintético de 11 páginas/máxima complejidad (46/46 + 48/48 aserciones XML + validación SAX de buena formación); sin E2E en vivo contra Word/Excel/PowerPoint reales todavía (2026-09-22) | Enruta el export DOCX al backend (antes solo corría en el cliente pese a existir la ruta) con fallback a ADR-139 si el sidecar falla. Nuevo modo de layout `flow` (texto que fluye/reflowa, alternativa al `absolute` de ADR-139) con paridad completa: motor de fórmulas/formato condicional/semántico portado al sidecar (`tableEngine.js`, mismo criterio que ADR-172), listas nativas de Word (`w:numPr`, numeración/avance automático real, no texto plano), tachado e hipervínculos de ida y vuelta (import+export, ambos pipelines). Cierra 6 bugs reportados por un compañero del equipo (fórmulas crudas, listas, colores, espaciado, escalado de carátula/imagen A3) más 3 gaps de import no reportados (tamaño/orientación de página, estilo "Title", resaltado) encontrados en la misma auditoría. Bug real encontrado y corregido en la propia verificación: `sanitizeSpans` (cliente) descartaba en silencio `strikethrough`/`href` por ser una whitelist campo-por-campo nunca actualizada — el código nuevo estaba correcto en disco pero no llegaba a producir efecto hasta corregir el sanitizador. |
| 206 | `wordart-columnas-texto-motor-tablas-pptx-brechas-cruzadas-docx-pptx` | ✅ implemented, verificado con export real (Node directo contra los 3 builders) + inspección XML/OOXML + validación SAX de buena formación (22+22+54 partes) + render real en navegador (ReadOnlyViewer isPrint=true) contra documento de 9 páginas (2026-09-23) | Dos funcionalidades NUEVAS del editor (no existían): WordArt (texto decorativo con degradado/contorno/sombra, `WordArtBlock.tsx`/`ReadOnlyWordArt`, capturado como raster para DOCX/PPTX igual que un gráfico) y columnas de texto (`props.columnCount`, CSS real en pantalla/PDF, aproximado con N cuadros lado a lado en DOCX/PPTX -- deliberadamente SIN habilitar en el lienzo interactivo por riesgo sobre el sistema de auto-paginación). Misma auditoría encontró y cerró 4 brechas cruzadas entre formatos: `shape` (diagramas de bloque) faltaba en los 3 builders DOCX/PPTX; la Leyenda de imagen (`props.caption`) nunca se exportaba pese a pintarse siempre en pantalla/PDF; `resolveAnnexRefLabel` nunca se conectó al pipeline PPTX (mismo bug que ADR-204 ya había cerrado solo para DOCX); y el hallazgo más grande -- el motor de fórmulas/condicional/semántico/colores manuales de tabla (ADR-172, portado a DOCX en ADR-204) nunca se había portado a PPTX, que hasta ahora exportaba `=SUM(...)` como texto crudo sin ningún color. |
| 209 | `importacion-pdf-modo-replica-sin-superposiciones` | ✅ implemented, verificado en vivo con 3 PDF reales (68/132/112 págs.) importados por la UI y auditados en el visor (2026-09-23) | Importación PDF en modo réplica: fondo por página sin texto + texto editable posicionado por línea base real, escala uniforme, orientación por página, sin encabezado/pie de plataforma ni auto-flujo (`fixedLayout`/`exactLayout`). Corrige 750 superposiciones del borrador, redacción parcial de MuPDF que corría texto (hoja 14 Reglamento) y texto oculto tapado por imágenes. Amplía ADR-199. |

**Ámbito `reports`: 47/51 implemented/accepted (40 si se cuenta ADR-173 como implemented pese a sus hallazgos abiertos, +5 con ADR-179/181/182/184/185, +2 con ADR-201/204; ADR-176 es `proposed` a propósito, no decide nada; ADR-183 es `measured` a propósito — documenta un criterio real que NO se cumple, no cuenta como implemented/accepted; ADR-185 es `accepted` — decisión de negocio que cierra SPEC-007 T15/O3, cuenta como accepted pese a documentar que el valor literal de la SOW no se cumple), 1 pendiente E2E y 1 superseded** (ADR-019 pasó de partial a implemented el 2026-08-05, ver su actualización; ADR-139 cerró su pendiente E2E el 2026-09-02; ADR-172/173/174/176 formalizados 2026-09-12; ADR-079 es ámbito `plataforma`, no `reports` — su fila ya está reconciliada en la tabla de `plataforma` arriba; ADR-201/204 pendientes de E2E en vivo contra Word/Excel/PowerPoint/Acrobat reales, contados igual como implemented por la verificación directa de XML/build ya hecha). Ver "Progreso del proyecto de reportabilidad" abajo (cálculo no recalculado en esta pasada para 080/083/084 — cubre 010-073 + 019).
...
| 089 | `biometria-dermalog-cli-integration` | 🔄 partially superseded by ADR-105; SDK descartado por licencia comercial (ADR-180, 2026-09-12) | SDK comercial Dermalog Face (`BiometricProvider::DermalogCli`, subproceso vía `dermalog-face-cli`) como validador biométrico principal por exigencia de cumplimiento/certificación; InsightFace (ArcFace) pasa a fallback automático si el binario Dermalog falla. Sustituye formalmente `specs/adr/ADR-005` (que ya tiene el banner `SUPERSEDED` correcto apuntando aquí). Degradado a secundario explícito por ADR-105 (2026-08-12); licencia comercial descartada por costo por ADR-180 (2026-09-12) — el wrapper CLI se conserva en el código, inactivo por defecto. |
...
| 151 | `numero-reservado-sin-uso` | ⚪ N/A — número retirado, sin decisión asociada (2026-09-11) | Hueco de numeración real entre ADR-150 y ADR-153, encontrado en la auditoría 2026-09-10 y sin evidencia de contenido perdido (verificado con `git log --all`). Se retira formalmente en vez de renumerar los ADR 153+ ya citados en otros documentos y en commits pusheados. |
| 152 | `numero-reservado-sin-uso` | ⚪ N/A — número retirado, sin decisión asociada (2026-09-11) | Mismo tratamiento que ADR-151, hueco gemelo. |
...
| 167 | `avatar-soporte-alcance-v1-saludo-bienvenida-justificacion-ia` | ✅ accepted (alcance de negocio v1, 2026-09-11); hoja de ruta ampliada `proposed`/diferida | Fija el alcance v1 del avatar de soporte a **solo saludo de bienvenida** (`onboarding` en `AvatarWidget.tsx`) — soporte conversacional, avatar animado en producción, sistema de alarmas/advertencias con contenido propio y tutorial guiado quedan diferidos, no descartados (mismo patrón que ADR-025/EPP), requieren su propio SPEC/ADR. Los disparadores `report`/`alarm_loop` de ADR-164 siguen activos en código como refuerzo incidental, sin ser parte de la promesa v1. Documenta la justificación de la inversión: construir el avatar fue el vehículo real que forzó el endurecimiento de liveness/anti-spoofing (ADR-142,145,146,148,149,156), el reemplazo del tracking facial por MediaPipe (ADR-162), el aislamiento de SilentFace que resolvió un conflicto real de cuDNN (ADR-143), el pipeline de curación/reentrenamiento del clasificador de lentes que mejora el login real de producción (ADR-119/163/165), guardrails de registro (ADR-147/153/161) y el cierre de un hallazgo de licenciamiento real (ADR-144/166) — mejoras que sostienen la biometría de toda la plataforma, no solo el widget. |
| 180 | `descarte-dermalog-licencia-comercial-costo-adicional` | ✅ accepted (decisión de negocio, 2026-09-12) | Descarta la adquisición de la licencia comercial de Dermalog (ADR-089) por costo adicional sin caso de negocio, dado que DeepFace+Silent-Face-Anti-Spoofing (ADR-105) ya es el proveedor primario real, sin costo. Cierra SPEC-008 T16 (certificación con hardware real, S11/R5) — se retira del pendiente, no se reprograma. El wrapper CLI (`vendor/dermalog/`) se conserva en el código, inactivo por defecto (ya lo estaba desde ADR-105); sin ningún cambio en el sistema biométrico real de producción. Mismo patrón de decisión que ADR-166 (InsightFace). |
| 202 | `avatar-tts-neuronal-piper-reemplaza-espeak-ng` | 🟡 implemented en código, build/despliegue real pendiente de verificación (2026-09-20) | Reporte del usuario: el saludo de bienvenida del avatar (ADR-150/167) suena robotizado — causa confirmada en `sadtalker_backend.py::_synthesize_tts_wav`, que usaba espeak-ng (síntesis por formantes). Reemplazado por Piper (VITS neuronal sobre onnxruntime, `piper-tts==1.8.0`), voz `es_MX-claude-high` (Apache 2.0, verificado en su `MODEL_CARD` de Hugging Face). Sigue 100% local/offline, mismo criterio de licencia de audio ya resuelto por ADR-150 — Piper es GPL-3.0-or-later pero se invoca exclusivamente como subproceso (entry point `piper`, nunca `import piper`), mismo aislamiento ya aplicado a espeak-ng. Wheel manylinux prebuilt: ya no hace falta el paquete apt `espeak-ng` (retirado del `Dockerfile`); voz descargada bajo demanda al volumen persistente ya existente. |
| 203 | `avatar-recorte-cabeza-catalogo-cuerpo-vestimenta` | 🟡 implementado y verificado por build real (backend Docker+ctest verde, frontend `tsc`+`vite build` verde, `ai_engine` reconstruido y probado en vivo dentro del contenedor real); pendiente verificación visual con foto de registro real (2026-09-20) | Pedido explícito del usuario: dejar de depender de segmentar hombros/ropa fotografiados (fuente de varios incidentes reales, ADR-157/158, y de los parches sucesivos de `avatar_animation_engine/matting.py`) — el avatar recorta SOLO la cabeza (nuevo segmentador multiclase de MediaPipe, pelo/piel-cara/piel-cuerpo/otros, TFLite/CPU sin costo de GPU, corte de cuello por línea fija de landmarks) y la compone sobre una de 4 plantillas de cuerpo/vestimenta pre-hechas dibujadas por código (no difusión, no arte de terceros — mismo criterio que el logo de ADR-164). Nuevo recorte de cabeza persistido (`/data/auth/avatar_heads/{userId}.png`) permite "cambiar de vestimenta" como un recompose instantáneo sin GPU/difusión (`POST /api/auth/avatar/body-template`). Avatar flotante (`AvatarWidget.tsx`) reducido 50% (220px→110px). Encontró y corrigió un bug real de alineación (ancla al tamaño del arreglo en vez del bounding box real del alfa) con una prueba sintética antes de darlo por bueno. |
| 207 | `fotocheck-empresa-minera-area-trabajo-rediseno` | 🟡 implemented, verificación en curso (2026-09-23) | Fotocheck: el encabezado muestra la empresa minera del tenant del usuario (ya no "BEEMETRY"); solo Nombres/Apellidos/DNI visibles (QR cifrado sin cambios); color y banda de área de trabajo por rol (7 roles); render Pillow + Inter (`fonts-inter`) y QR a escala entera por módulo. |

**Ámbito `ia`: 41 filas reales en esta tabla al 2026-09-12 (+1 con ADR-180; recontadas por grep de filas `| NNN |` desde el "40" del 2026-09-11), +1 con ADR-202 (2026-09-20), +1 con ADR-203 (2026-09-20). Desglose no recalculado fila por fila en esta pasada; ver cada fila individual para status exacto.**
...
| 190 | `auditoria-coordenadas-gps-oficiales-empresas` | ✅ implemented, aplicado en vivo (2026-09-15) | Rediseño completo de `CompanyLocationPicker.tsx`: pin propio con glifo de montaña (fix del icono roto de Leaflet bajo Vite), campos de Latitud/Longitud editables en pantalla sincronizados en tiempo real con el arrastre del marcador, selector Satélite/Plano (Google), y modo solo-lectura para `empresas.view` sin `empresas.manage`. Auditoría de las 51 coordenadas de mina de ADR-121 contra fuentes oficiales (MINEM/DGAAM, SENACE, informes técnicos regulatorios): 35 corregidas con fuente citada (`db_scripts/97`), 17 sin cambio por falta de fuente verificable — sin inferencia de IA (principio de ADR-121). Addendum 1: corrección de `domicilio_fiscal` (texto) para 4 empresas más y cruce Ares/Inmaculada con Hochschild Mining Peru (`db_scripts/98`). Addendum 2: reemplazo del RUC sintético (ADR-088) por el RUC real de 48/52 empresas, cada uno verificado EN VIVO contra la integración SUNAT ya activa (ADR-087, api.chequea.pe — se descubrió que sí estaba configurada, corrigiendo una nota previa desactualizada de este registro); `domicilio_fiscal` ahora es la dirección legal real de SUNAT, separada del concepto de ubicación física de la mina (`latitude`/`longitude`). Un candidato (Alpayana) falló la verificación en vivo y se descartó (`db_scripts/99`). También se reconstruyó y redesplegó el contenedor Docker del frontend, que corría una build vieja sin ninguno de estos cambios. |
| 196 | `mapa-sensores-estado-compuesto-tiempo-real` | 🟡 implemented (backend+frontend), verificación E2E/carga WS pendiente del usuario (2026-09-17) | `/api/map/markers` y `MapAggregator` ahora devuelven `status` compuesto (severidad de alarma activa de `platform_alarms` sobre `connection_status`) + `sensor_type`; `MapViewer.tsx` colorea por severidad y muestra ícono+etiqueta (`divIcon`) por tipo de instrumento cuando el volumen visible es bajo (≤200, zoom≥15). Addendum: clic en un sensor abre popup interactivo con último valor + sparkline vía `wizard/query` (endpoint ya existente, reusado), con polling acotado a ese sensor mientras el popup sigue abierto y reapertura automática si el marcador se recrea por cambio de estado. No se migró el modelo de concurrencia del WS del mapa sin evidencia real — se entregó `scripts/map-ws-load-test.js` (mismo método que la prueba de 200 conexiones de ADR-181) para que el usuario lo corra en su propia terminal. |

**Ámbito `geo`: 7 activos implementados (1 con verificación de carga pendiente) y 1 superseded (ADR-028).**
...
### Ámbito `marketing` — página pública (promoción/marketing/publicidad)

Segundo ámbito nuevo agregado desde `soporte` (2026-09-11). Cubre
`frontend/src/components/Home/` — la página pública pre-login de BEEMETRY:
landing, roadmap público, dashboard "en vivo" y demo biométrica simulados.
Llegó al repositorio sin ADR propio; formalizado retroactivamente junto con
la corrección de tres hallazgos de contenido de una auditoría de
conformidad (ver ADR-171).

| # | Slug | Status | Resumen |
|---|---|---|---|
| 171 | `pagina-publica-marketing-datos-ilustrativos` | ✅ implemented (2026-09-11) | Formaliza `Home/` como página pública de marketing/promoción, fuera de la autenticación. Regla dura: todo dato de persona/empresa en esta página es ficticio mientras no haya clientes reales (ADR-134) — cierra tres hallazgos reales: testimonios atribuidos a mineras reales (Alpayana/Cerro Verde/Antamina) sin aviso, demo de reconocimiento facial simulada sin declararlo, y el nombre real de un desarrollador filtrado en el estado de demo. |

**Ámbito `marketing`: 1/1 implemented.**
...
### Auditoría y plan maestro (2026-09-23)

| # | Slug | Estado | Resumen |
|---|---|---|---|
| 210 | `auditoria-integral-2026-09-23-cierre-adr-y-conflictos` | ✅ implemented (acciones H1-H3 abiertas para el developer) | Verificación ADR↔implementación; 8 hallazgos; cierre de 7 ADR |
| 211 | `saneamiento-ledger-migraciones-version-81-duplicada-y-seeds-dev` | ✅ accepted; implementación pendiente (S9) | Runner bloqueado en v81; 115/116 sin aplicar; separar seeds dev |
| 212 | `plan-maestro-2026-09-23-pendientes-riesgos-decisiones-gerencia` | ✅ accepted; decisiones G-1…G-17 abiertas | Backlog fino ≈47, pistas A-E, riesgos, escenarios de fecha, go/no-go |

### Fecha referencial de término

**Actualizado 2026-09-23 (ADR-212).** El objetivo de julio 2026 quedó superado.
Ventana de go-live vigente: **S13, 2026-11-27, condicionada** a pentest externo
sin hallazgos críticos/altos, simulacro DR cronometrado, restore en host limpio,
UAT firmado, marcha blanca y rollback ensayado. Plan B recomendado: ventana
posterior (propuesta: semana del 2026-12-07) si la contratación del pentest o
el simulacro DR se retrasan. Detalle en ADR-212 y
[`docs/PLAN_MAESTRO_2026-09-23.md`](../PLAN_MAESTRO_2026-09-23.md).

*Texto histórico (2026-07/08), conservado por trazabilidad:* el estimado anterior
era fines de julio 2026; la auditoría del 2026-07-24 lo invalidó por el 1,7%
arquitectónico de ADR-019 (cerrado el 2026-08-05) y por el backlog operativo
(pentest 1-2 semanas de calendario una vez agendado; QA funcional + go-live
3-5 días).
