...
| 058 | `auditoria-seguridad-integral-jul2026` | ✅ implemented (2026-07-19) | XSS almacenado e IDOR sin auth cerrados; CVEs altas llevadas a 0 y HSTS agregado. Pendientes: pentest externo y `echarts@6` (1 CVE moderada). |
...
| 077 | `migracion-password-argon2id-versionada` | ✅ implemented · transición legacy operativa | Argon2id + rehash oportunista; inventario 14 Argon2id/40 legacy tras smoke del 2026-07-29. |
...
**Ámbito `plataforma`: 27/29 implemented, 1 partial, 1 proposed** (conteo previo a 079-081; ver ADRs individuales para su status).
...
| 019 | `resolucion-diferida-numeracion-toc-refs` | ⚠️ partial (2026-07-24) | Numeración/TOC/"Página X de Y" implementados; referencias cruzadas automáticas aún no existen. |
...
**Ámbito `reports`: 31/33 implemented, 1 partial y 1 superseded** (más ADR-079, ámbito `plataforma` mismo dominio — ver nota de auditoría arriba; tabla aún no reconciliada para 079/081, que no cambian este subtotal de `reports`). Ver "Progreso del proyecto de reportabilidad" abajo (cálculo no recalculado en esta pasada — cubre 010-073, no incluye 080/083/084).
...
**Ámbito `ia`: 3 implemented y 1 deferred por diseño (EPP no cuenta como pendiente v0.1).**
...
| 072 | `gdal-cli-runtime-admin-confinado` | ⚠️ partial; código implementado, rebuild final y QA raster pendientes (2026-07-24) | GDAL CLI on-demand admin-only, rutas confinadas a `/data`, parámetros allowlist y jobs autenticados/tenant-owned. |

**Ámbito `geo`: 2 activos implementados, 1 partial y 1 superseded (ADR-028).**
...
| Estado | ADRs | Peso |
|---|---|---|
| Implementado sin reservas (incl. alcance v0.1 explícitamente reducido por diseño) | 010, 011, 012, 013, 014, 015, 016, 017, 018, 020, 021, 022, 039, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 055, 064, 065, 070, 071, 073 | 29 × 1.0 = 29.0 |
| Parcial | 019 (faltan referencias cruzadas automáticas) | 1 × 0.5 = 0.5 |
| **Total** | **30 ADRs** | **29.5 / 30** |

### → **Avance del proyecto de reportabilidad: 98.3%** (29.5 / 30 ADRs activos)
...
- **Retirar el verificador legacy ADR-077** cuando su inventario llegue a
  cero; las 40 cuentas históricas migran al login/reset sin interrupción.
...
- QA funcional formal v0.1 (cobertura parcial vía smoke tests repetidos;
  falta un pase exhaustivo firmado, incluida cámara/pantalla y una conversión
  raster GDAL con inspección visual).
...
La auditoría del 2026-07-24 invalida tratar esa fecha como lista para
producción: además del 1.7% arquitectónico pendiente de ADR-019, quedan los
gates y bloqueantes del backlog anterior.
...
como compromiso: Argon2id y ECharts ya están cerrados, pero exige completar
QA, ADR-019 y no recibir hallazgos críticos del pentest.
